...
import javax.swing.*;
/**Disegna un labirinto dal suo interno e permette di muoversi con le frecce direzionali*/
public class graphicMaze3D extends graphicMaze implements KeyListener, java.io.Serializable{
...
/**Crea un componente inizialmente privo di labirinto.
  *Fino a che non viene invocato il metodo newMaze
  *il componente visualizza l'immagine "maze2.jpg"
  */
 ...
 protected void drawMaze(){
 	...
 	if(maze == null) return;
	//creo un buffer quadrato scegliendo la dimensione più
	//restrittiva tra la larghezza del componenete e la sua altezza
 	bufferHeight = bufferWidth = getWidth() < getHeight() ? getWidth() : getHeight() ;
	buffer = new BufferedImage( bufferWidth,bufferWidth,BufferedImage.TYPE_INT_RGB);
...
/**Disegna la vista corrente. */
 private void updateMaze(){
	...
	//in base a dove è rivolto il giocatore definisco i
	...
	int[] ly = {-insideCell,-insideCell,bufferWidth+insideCell,bufferWidth+insideCell};
	int[] rx = {bufferWidth+insideCell,bufferWidth+insideCell,bufferWidth+insideCell,bufferWidth+insideCell};
	int[] ry = {-insideCell,-insideCell,bufferWidth+insideCell,bufferWidth+insideCell};
	int z = 2;
	...
	while( current!=null ){
		...
		if(leftSide == null){//se c'e' il muro
			lx[0] = lx[1]; 		 ly[0] = ly[1];
			lx[1] += cellSide / z; ly[1] += cellSide / z;
			lx[3] = lx[2]; 		 ly[3] = ly[2];
			lx[2] += cellSide / z; ly[2] -= cellSide / z;
		...
		lx[0] = lx[1]; 		 ly[0] = ly[1];
		lx[1] += cellSide/4 / z; ly[1] += cellSide/4 / z;
		lx[3] = lx[2]; 		 ly[3] = ly[2];
		lx[2] += cellSide/4 / z; ly[2] -= cellSide/4 / z;
		if( ly[1] > bufferWidth/2)
			ly[1] = ly[2] = lx[1] = lx[2] = bufferWidth/2;
		...
		if(rightSide == null){//se c'e' il muro
			rx[0] = rx[1];		 ry[0] = ry[1];
			rx[1] -= cellSide / z; ry[1] += cellSide / z;
			rx[3] = rx[2]; 		 ry[3] = ry[2];
			rx[2] -= cellSide / z; ry[2] -= cellSide / z;
		...
		//disegno lo spessore dei muri
		rx[0] = rx[1];		 ry[0] = ry[1];
		rx[1] -= cellSide/4 / z; ry[1] += cellSide/4 / z;
		rx[3] = rx[2]; 		 ry[3] = ry[2];
		rx[2] -= cellSide/4 / z; ry[2] -= cellSide/4 / z;
		if( ry[1] > bufferWidth/2 )
			ry[1] = ry[2] = rx[1] = rx[2] = bufferWidth/2;
	...
	b.fillRect(lx[0],ly[0],rx[0]-lx[0],ly[3]-ly[0]);
		//scrivo dove è rivolto il giocatore
	...
	b.setColor(Color.black);
	b.fillRect(player.column*zoom+1,player.row*zoom+1,zoom-1,zoom-1);
	b.setPaintMode();
...
/**Quando vengono premute le frecce direzionali viene cancellata
  *la vecchia posizione del giocatore e viene disegnata quella nuova
  */		
 public void keyPressed(KeyEvent e){
	     //se sono alla fine non mi muovo più
	if( player == exit && lookAt==direction.east) return;

	switch(e.getKeyCode()){
 		case KeyEvent.VK_DOWN:  lookAt = (lookAt + 2) % 4;
					move();
					lookAt = (lookAt + 2) % 4;
 					break;
 		case KeyEvent.VK_LEFT:  lookAt = (lookAt + 3) % 4;
					break;
 		case KeyEvent.VK_RIGHT: lookAt = (lookAt + 1) % 4;
 					break;
 		case KeyEvent.VK_UP:    move();
					break;
 	}
