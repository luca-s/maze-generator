...
public class MazeGUI extends JFrame{
 ...
 public MazeGUI(){
	...
	setJMenuBar( creaMenuBar() );//aggiungo la MenuBar
 ...
 private JMenu file(){
	...
	save.addActionListener(new ActionListener(){
				 public void actionPerformed(ActionEvent e){
				 	boolean saved = true;
					FileDialog save = new FileDialog(MazeGUI.this,"Save state",FileDialog.SAVE);
     					save.setVisible(true);
     					if(save.getFile()!=null)
						if(is3D)
					  		saved = labirinth3D.saveMaze(save.getDirectory()+save.getFile());
						else saved = labirinth2D.saveMaze(save.getDirectory()+save.getFile());
					else if(!saved){ JOptionPane.showMessageDialog(null,"Impossibile salvare il file");} } } );
        JMenuItem load = new JMenuItem("Load...");
	load.addActionListener(new ActionListener(){
				 public void actionPerformed(ActionEvent e){
				  	boolean loaded = true;
				 	FileDialog load = new FileDialog(MazeGUI.this,"Load state",FileDialog.LOAD );
					load.setVisible(true);
					if(is3D)
					  loaded = labirinth3D.setMaze(load.getDirectory()+load.getFile());
					else loaded = labirinth2D.setMaze(load.getDirectory()+load.getFile());
					if(!loaded){ JOptionPane.showMessageDialog(null,"Impossibile caricare il file");} } } );
 ...
 private JMenu action(){
	...
	newMaze.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_N, ActionEvent.ALT_MASK));
	newMaze.addActionListener(new ActionListener(){
				 public void actionPerformed(ActionEvent e){
				 	if(is3D)
				 	  new newMazeDialog(MazeGUI.this,"Set Dimension",true,labirinth3D);
				 	else
					  new newMazeDialog(MazeGUI.this,"Set Dimension",true,labirinth2D);} } );
...
//crea il menu view
 private JMenu view(){
	...
	overview.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_O, ActionEvent.ALT_MASK));
	overview.addActionListener(new ActionListener(){
				 public void actionPerformed(ActionEvent e){
				 	MazeGUI.this.over();} } );
	JMenuItem insideview = new JMenuItem("Inside view ");
	insideview.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_I, ActionEvent.ALT_MASK));
